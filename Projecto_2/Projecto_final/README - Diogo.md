...
**Criar utilizador**  
```bash
cd  C:\code\aulas_programacao\Projecto_2\Projecto_final
``` 
```bash
python main.py create-user --username diogo
``` 
...
```bash
python main.py login --username diogo --ip 192.168.1.10
```
...
**Registo de logs:**
Cada tentativa (sucesso, falha) é guardada em `logs_exemplo.csv`:
```
...
Ferramenta: `python -m safety check`  
